...
class RuntimeOptions:
    ...
    def __repr__(self) -> str:
        """Default dataclass implementation doesn't consider plugin applied options."""
        r = ", ".join(f"{name}={option}" for name, option in sorted(vars(self).items()))
        return f"{self.__class__.__name__}({r})"

    def __post_init__(
        self, pluginOptions: Optional[dict[str, RuntimeOptionValue]]
    ) -> None:
        """
        ...
        if pluginOptions:
            ...
            if existingBaseOptions:
                raise RuntimeOptionsException(
                    _(
                        "Provided plugin options already exist as base options {}"
                    ).format(existingBaseOptions)
                )
            for optionName, optionValue in pluginOptions.items():
                setattr(self, optionName, optionValue)
        if (
            self.entrypointFile is None
            and not self.proxy
            and not self.plugins
            and not pluginOptions
            and not self.webserver
        ):
            raise RuntimeOptionsException(_("Incorrect arguments"))
        if self.webserver and not hasWebServer():
            raise RuntimeOptionsException(
                _("Webserver option requires webserver module")
            )
