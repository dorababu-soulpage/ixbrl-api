...
def create_presentation_loc_element(parent_tag=None, label=None, xlink_href=None):
...
# Create presentation root element
...
# Add presentation loc elements
loc_cover_abstract = create_presentation_loc_element(
...
loc_entities_table = create_presentation_loc_element(
    parent_tag=presentation_link,
...
# Add presentation arc elements
presentation_arc = create_presentation_arc_element(
    parent_tag=presentation_link,
    order="1",
...
xml_data = (
    ...
    + "\n"
    + ET.tostring(presentation_link, encoding="utf-8").decode()
)

# Print or use xml_data as needed

# Optionally, write the XML to a file
with open("pre.xml", "w", encoding="utf-8") as file:
    file.write(xml_data)
